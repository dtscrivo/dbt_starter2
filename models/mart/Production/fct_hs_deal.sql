...
--friday
 ...
 -- , case when pay_type = "PIF" AND num_payments_made > 1 then 1 else 0 end as extra_pif
  ...
  , case when id_owner = 1426033370 AND (date(date_closed) > date('2024-08-23') AND date(date_closed) < date('2024-08-28')) then "Charlotte_8/24"
         ...
         when id_owner IN (1426033370,368848093,400703992,746953962,863704292,399470310,1297927104) AND (date(date_closed) >= date('2025-7-24') AND date(date_closed) <= date('2025-8-11')) AND name_product like "%Mastermind Business Academy%" THEN "WS25"
